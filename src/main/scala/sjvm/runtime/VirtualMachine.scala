...
class VirtualMachine(cmdlineArguments: CmdlineArguments) {
  ...
  private def readyClass(className: String): Unit = {
    val pathToClassFile = s"${cmdlineArguments.classpath}/$className.class"
    val jclass = Parser.parse(pathToClassFile, cmdlineArguments.parseVerbose)
  ...
  private def eval(className: String, instruction: Instruction, i: Int): Int = {
    ...
    instruction.opcode match {
      ...
      case "lrem" =>
        val x1 = pop().asInstanceOf[Long]
        val x2 = pop().asInstanceOf[Long]
        push(x2 % x1)
